...
def GetQueryPaloAlto(appServer, user, password, name, inXml, starttime, endtime):
	...
	if content.decode("utf-8")=='100':
		UrlFinal=Url+'events/'+queryId+'/0/600'
...
def FlaskServer(xml,s,e):
	...
	data=GetQueryPaloAlto(appServer,user, password, 'all', xml, s, e)
	return generateResult(data)

if __name__=='__main__':
	import sys
	if len(sys.argv)!=4:
		print json.dumps({'alert':"Uso: GetQueryPaloAlto.py inputXML starttime endtime"})
		exit()
